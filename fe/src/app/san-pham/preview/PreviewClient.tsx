...
import { CheckCircle2, ShieldCheck, Truck, Zap, Gift, ArrowRight, RefreshCcw, Settings, Eye, Maximize, Cpu, Monitor, Server, HardDrive, MemoryStick, Gpu, Battery, Layers, Keyboard, Mouse, Link as LinkIcon } from "lucide-react";
...
export default function ProductPreviewClient({ product }: { product: any }) {
  ...
  if (specArray.length === 0 && cleanDescription) {
    ...
    for (const line of lines) {
      if (line.includes('- CPU:') || line.includes('- RAM:')) {
        const parts = line.split('- ').filter((p: any) => p.trim() && p.includes(':'));
              ...
              {product.gifts && product.gifts.length > 0 && (
                <div className="mb-8 border border-orange-200 rounded-2xl overflow-hidden bg-orange-50/30">
                  ...
                  <div className="p-5 flex flex-col gap-3 text-sm text-gray-700">
                    {product.gifts.map((gift: any, idx: any) => {
                      return (
