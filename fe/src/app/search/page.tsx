                  ...
                  {['ASUS', 'MSI', 'GIGABYTE', 'DELL', 'HP', 'LENOVO', 'APPLE'].map((brand) => (
                    <label key={brand} className="flex items-center gap-3 cursor-pointer group/label">
                      <div className="relative flex items-center justify-center w-5 h-5">
                        <input type="checkbox" name="brand" value={brand} defaultChecked={brands.includes(brand)} className="peer appearance-none w-5 h-5 border-2 border-gray-300 rounded focus:ring-0 checked:bg-red-500 checked:border-red-500 transition-all cursor-pointer" />
                        <svg className="absolute w-3 h-3 text-white pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                      </div>
                      <span className="text-gray-600 text-[14px] font-medium group-hover/label:text-red-600 transition-colors">{brand}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2.5">
                {[
                  ...
                  <label key={price.id} className="flex items-center gap-3 cursor-pointer group/label">
                    <div className="relative flex items-center justify-center w-5 h-5">
                      <input type="radio" name="price_range" value={price.id} defaultChecked={priceRange === price.id} className="peer appearance-none w-5 h-5 border-2 border-gray-300 rounded-full focus:ring-0 checked:border-red-500 transition-all cursor-pointer" />
                      <div className="absolute w-2.5 h-2.5 bg-red-500 rounded-full pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity scale-0 peer-checked:scale-100"></div>
                    </div>
                    <span className="text-gray-600 text-[14px] font-medium group-hover/label:text-red-600 transition-colors">{price.label}</span>
                  </label>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-col gap-2.5">
                {['Mới 100%', 'Cũ (Like New)'].map((condition) => (
                  <label key={condition} className="flex items-center gap-3 cursor-pointer group/label">
                    <div className="relative flex items-center justify-center w-5 h-5">
                      <input type="checkbox" name="condition" value={condition} defaultChecked={conditions.includes(condition)} className="peer appearance-none w-5 h-5 border-2 border-gray-300 rounded focus:ring-0 checked:bg-red-500 checked:border-red-500 transition-all cursor-pointer" />
                      <svg className="absolute w-3 h-3 text-white pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                      </svg>
                    </div>
                    <span className="text-gray-600 text-[14px] font-medium group-hover/label:text-red-600 transition-colors">{condition}</span>
                  </label>
                ))}
              </div>
            </div>

            <button type="submit" className="w-full py-3 bg-red-50 hover:bg-red-500 text-red-600 hover:text-white font-bold rounded-xl transition-all duration-300 border border-red-100/50 hover:shadow-[0_4px_15px_rgba(239,68,68,0.3)]">
              Áp dụng bộ lọc
            </button>
          ...
          {products.length === 0 ? (
            ...
            <>
              <div className="mb-6 flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                <span className="font-bold text-gray-800">Tìm thấy <span className="text-red-600 font-black text-lg mx-1">{products.length}</span> sản phẩm</span>
              </div>
                    ...
                    >
                      ...
                      <div className="relative aspect-[4/3] p-4 flex items-center justify-center bg-white overflow-hidden">
                        ...
                        <div className="absolute inset-0 pointer-events-none z-[15] p-2 opacity-80">
                          <div className="w-full h-full border border-red-500/10 rounded-xl relative group-hover:border-red-500/30 transition-colors duration-500">
                            <div className="absolute -top-[1px] -left-[1px] w-5 h-5 border-t-2 border-l-2 border-red-500/40 rounded-tl-xl group-hover:border-red-500/80 transition-colors"></div>
                            <div className="absolute -top-[1px] -right-[1px] w-5 h-5 border-t-2 border-r-2 border-red-500/40 rounded-tr-xl group-hover:border-red-500/80 transition-colors"></div>
                            <div className="absolute -bottom-[1px] -left-[1px] w-5 h-5 border-b-2 border-l-2 border-red-500/40 rounded-bl-xl group-hover:border-red-500/80 transition-colors"></div>
                            <div className="absolute -bottom-[1px] -right-[1px] w-5 h-5 border-b-2 border-r-2 border-red-500/40 rounded-br-xl group-hover:border-red-500/80 transition-colors"></div>
                        ...
                        {!isOutOfStock && (
                          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 translate-y-8 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300 z-30 pointer-events-none">
                            <div className="bg-white/95 backdrop-blur-sm text-red-600 text-sm font-bold px-6 py-2 rounded-full shadow-lg border border-red-600/20 flex items-center gap-2 whitespace-nowrap">
