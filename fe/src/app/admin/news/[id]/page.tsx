...
export default function EditNewsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const calculateSEO = () => {
    ...
    if (wordCount >= 300) {
      ...
      checks.push({ text: `Nội dung quá ngắn (${wordCount} từ, nên > 300)`, pass: false });
    }
    ...
    if (focusKeyword.trim()) {
      ...
      if (keywordCount >= 2) {
        score += 15;
        checks.push({ text: `Từ khóa có trong bài viết (${keywordCount} lần)`, pass: true });
      } else {
        checks.push({ text: "Từ khóa nên xuất hiện ít nhất 2 lần", pass: false });
      }
    } else {
    ...
    if (hasImages) {
      if (focusKeyword.trim()) {
        ...
        const hasAltKeyword = imgTags.some(img => img.toLowerCase().includes(keyword));
        if (hasAltKeyword) {
          score += 10;
          checks.push({ text: "Ảnh có chứa từ khóa (Alt/Title)", pass: true });
        } else {
          checks.push({ text: "Nên thêm từ khóa vào tên ảnh hoặc Alt", pass: false });
        }
      } else {
        score += 10;
